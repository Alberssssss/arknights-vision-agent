...
- At this earlier checkpoint, a new-clone test and remote publication were pending. Two bounded GitHub Git-ref lookups timed out; no new push had been attempted or claimed. The 2026-09-08 checks below supersede that publication status, not the dated test evidence.

### Portable continuation publication (2026-09-08)

- A fresh non-local-copy Git clone of the local development branch checked out `bb15e443a51d5fc455993bfbbef7a545b3e060b7` without old environments or media artifacts. All 66 tracked files matched their Git blob bytes and modes before and after verification. No object alternates or source-object hard links were used. All eight entry documents and 92 local links in 31 Markdown documents were checked.
- The fresh local clone passed 307 application tests in 6.428s, nine developer-helper tests in 9.941s, compilation of `src`, `tests` and `tools/media_runtime`, and diff checks. All 24 recorded commands exited 0. The coordinator independently checked the retained command/log hashes, Git tree and four synthetic reports. The coordinator also reran the original checkout's 307 application tests in 6.347s. These are source-checkout checks on the same CPython 3.13.7/macOS arm64 host, not a new Codex client, installed-package, cross-platform or H20 test.
- Retained synthetic results: replay has three events with no executed action or verified clear; preparation assigns three groups with zero eligible records/groups; inventory hashes three text assets/77 bytes and finds one two-file duplicate candidate, with only `file_bytes_hashed` true; preflight retains the ordered 12 unmeasured identifiers and five false execution flags. Its four executable PATH-presence booleans were false. No model, device, native media or training operation was involved in these clone checks.
- Normal GitHub connectivity recovered. After verifying both remote tips were ancestors of the reviewed checkpoint and a matching dry-run succeeded, an atomic non-forced push published `bb15e44` to `main` and `codex/offline-foundation` (exit 0, 4.215s). A subsequent real remote lookup confirmed default `HEAD` is `refs/heads/main` and both refs point to the full checkpoint above. No access, authentication, global network, or CI configuration was changed; the development branch and local checkout are preserved.
- A subsequent genuine GitHub clone, using the repository URL without any branch argument, exited 0 in 3.057s and selected `main` at `bb15e44`; `origin/HEAD` selected `origin/main`, and both fetched branch refs matched. Its initial files contained no old `work/`, environments, binaries, recordings, model files or skills. All 66 tracked files matched the fixed Git tree before/after; eight entry documents and 92 local links were checked. All 30 logged commands exited 0 without a limit outcome: 307 application tests in 6.498s, nine helper tests in 9.928s, compilation, diff checks and all four synthetic examples with the same semantic boundaries above.
- The coordinator directly rechecked the network clone's branch/refs, clean worktree, all 66 Git blob identities/modes, 60 retained command-log byte/hash pairs and five report hashes, and inspected the actual four reports and replay events. Private audit ledger SHA-256: `806bffdbb5d0682d19916fe6d5267a9f5fe50bc0a2d2e539839de3133badfa33`. Raw paths/logs remain ignored. This closes the repository fresh-clone/publication check, not a test of a newly launched Codex/VS Code client, installed package, other Python/OS version, native decoder or H20 machine. All clone-audit commands finished; none is an in-flight continuation dependency.
- The three-document evidence closeout received separate specification/factual and quality reviews, then was committed as `35fd9bd41a17f33b67ba40dd3f0c762969403633`. A normal atomic push exited 0 in 2.123s; a subsequent lookup exited 0 in 1.078s and confirmed both main/development refs and default HEAD at that commit. This is a dated publication checkpoint, not a claim that it remains the newest ref after subsequent work.

### Developer-helper regression follow-up (2026-09-08)

- Reviewed test checkpoint: `5e5abf8bf28f0fa1a1da1bbc0945976c8255ada5`. Three previously one-off close-failure scenarios are now permanent tests: selector close, stdout close, and both together. They use actual children/selectors/pipes, inject an exception after the real close, and check leader reaping, later resource closure and retained exception identity/order before any test fallback.
- Only the developer test module changed. Its SHA-256 is `863d06db406bea6d20214cae6667cc8fddc2b5eaeb311286c5a8e63156f5a773`; the runner remains `3ebb2d087c8c2960f5becbae0e91a4c24f6d92d64be91570013f1265d9712a2c`. The coordinator verified `src`, application tests, runner and dependency declaration still match the GitHub-clone-tested `bb15e44` baseline. No production runner or native-index API was introduced.
- The coordinator freshly ran all 12 helper tests in 10.231s and all 307 application tests in 6.330s, with exit 0; compilation of `src`, `tests` and `tools/media_runtime` and diff checks also passed. The implementer had first confirmed the original nine-test baseline. Independent specification review passed with 12 tests in 10.245s; the separate final quality/integration review passed with 12 in 10.236s and 307 application tests in 6.362s.
- The coordinator independently reran the actual imported regressions with two in-memory faulty cleanup variants: fail-fast cleanup produced three assertion failures/zero errors for skipped later closes; dropping the second error produced exactly the combined test's one assertion failure/zero errors. It verified restoration of the real cleanup function, then all three new real-runner tests passed in 0.310s. These were deliberately failing diagnostics, not discovered defects in the unchanged runner. Original and reviewer diagnostic runs agreed; no tracked runtime file was replaced to run them.
- The quality reviewer additionally repeated the three new cases in randomized order for 30 executions, checked cleanup after a deliberately failed assertion, and observed its process-descriptor count unchanged at 4→4. These are reviewer-scoped checks, not coordinator reruns or a general leak/containment guarantee. Execution evidence remains CPython 3.13.7/macOS arm64; no native video decoder, model, game/device, private recording, GPU or training operation was involved.

### Native-index proposal, not implementation (2026-09-08)

- The [proposal](docs/native-index-proposal.md) records three approaches, the recommended narrow supervised ffprobe boundary, proposed limits, raw timing/geometry preservation and conservative completeness/error handling. H08 now records the required design-scope confirmation before a new product specification/plan and implementation. It does not waive H02/H03 or any device/GPU/training gate.
