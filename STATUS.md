...
The v1 manifest, metadata report, preparation command, byte inventory, and setup-preflight report are implemented and reviewed but do not constitute a complete media pipeline or a model/controller runtime. Continue native-runtime verification and build preparation, then separately bounded timestamp indexing and causal frame extraction from `docs/media-preparation-notes.md`. Downloaded-source identity checks are not build or decoder validation; no actual decoding calibration has been completed. Target/model/controller readiness still needs its own evidence and appropriate access. No actual training job should start automatically. See `HUMAN_HELP.md` for real recordings, target scope, and environment access.
