...
| H08 | Review the [native-index proposal](docs/native-index-proposal.md): narrow supervised local H.264 MP4/MOV indexing, raw timing/geometry and conservative evidence flags, synthetic inputs first | Complete the new product specification/plan, then test-first native-index implementation | Existing reviewed-plan follow-ups and read-only preparation; the three helper close-failure regressions have now been retained | Awaiting owner design-scope confirmation; no new data/device/training authorization |
