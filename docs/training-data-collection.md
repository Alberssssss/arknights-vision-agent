...
The implemented manifest validator checks declarations and lexical paths only. It does not establish data-use rights, resolve symlinks, inspect real media, verify label truth, or claim that a sample is ready for training. See [the exact v1 manifest contract](superpowers/specs/2026-09-07-demonstration-manifest-design.md) for the boundary. The implemented [metadata eligibility/group report](superpowers/specs/2026-09-07-preparation-report-design.md) and non-overwriting `prepare` command retain that distinction. The separate [byte inventory](superpowers/specs/2026-09-07-byte-inventory-design.md) reads only explicitly selected files and reports byte hashes/duplicate candidates; it does not decode media or establish timing, labels, or rights. Native indexing, extraction, and alignment remain future work, informed by [the video-input research](media-preparation-notes.md).
