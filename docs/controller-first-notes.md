...
流水线之前。**已确认的是优先级和 Windows/MuMu 目标描述，不是具体 SDK 设计或设备测试。**
...
这是用户提供的环境描述，不是本项目已经观测、连接或验证过的环境；具体
MuMu 版本未知，不能直接写成 MuMu 12 或套用某个示例端口、路径和多开实例。

第一项待答问题改为：该 Windows PC 上的 MAA 是否已经能正常连接 MuMu。
随后收集准确的 MuMu/MAA 版本、指定实例、客户端语言和截图尺寸，评审最小
接入设计及有限连接/截图/指定输入测试范围。H04 仅部分回答，H01 的主题、
难度、结局和队伍仍待确定；现有 MAA 能否工作也不能代替本项目适配器实测。
...
- MAA 的[连接说明](https://docs.maa.plus/zh-cn/manual/connection.html)
  ...
  的 MuMu 截图增强。先核实已有连接和准确版本，再评审采用哪种截图路径；
